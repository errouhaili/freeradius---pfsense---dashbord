...
DEFAULT_RADIUS_SECRET='K9x#mP2$vL8!qZ5&wN0@rT7*yU1^eX4='
echo "Appuie sur Entrée pour utiliser le secret par défaut du projet, ou tape le tien (min. 8 caractères)."
read -sp "Secret RADIUS partagé (clients.conf, pfSense <-> FreeRADIUS) : " RADIUS_SECRET
echo ""
...
# ---------------------------------------------------------------------
# Échappement sûr pour insertion dans des remplacements sed (s///).
# Sans ça, un mot de passe/secret contenant '&' ou '\' corromprait
# silencieusement le fichier de config généré (testé et confirmé).
# Ordre : 1) échapper le guillemet du fichier cible, 2) échapper les
# métacaractères de sed (\ puis &) pour que sed les recopie tels quels.
# ---------------------------------------------------------------------
sed_dq_escape() { # pour insertion dans une valeur "entre guillemets doubles"
    local s="${1//\"/\\\"}"
    s="${s//\\/\\\\}"
    printf '%s' "${s//&/\\&}"
}
sed_sq_escape() { # pour insertion dans une valeur 'entre guillemets simples'
    local s="${1//\'/\\\'}"
    s="${s//\\/\\\\}"
    printf '%s' "${s//&/\\&}"
}

DB_NAME_DQ="$(sed_dq_escape "$DB_NAME")"
DB_APP_USER_DQ="$(sed_dq_escape "$DB_APP_USER")"
DB_APP_PASS_DQ="$(sed_dq_escape "$DB_APP_PASS")"
RADIUS_SECRET_DQ="$(sed_dq_escape "$RADIUS_SECRET")"

DB_NAME_SQ="$(sed_sq_escape "$DB_NAME")"
DB_APP_USER_SQ="$(sed_sq_escape "$DB_APP_USER")"
DB_APP_PASS_SQ="$(sed_sq_escape "$DB_APP_PASS")"
...
if mysql --protocol=socket -u root -e "SELECT 1;" >/dev/null 2>&1; then
    ...
    error "  mysql -u root  # puis: ALTER USER 'root'@'localhost' IDENTIFIED BY '...';"
    exit 1
fi
...
# 5c. Utilisateur RADIUS de test (admin/admin), pour permettre à quiconque
#     exécute ce script de tester immédiatement l'authentification de bout
#     en bout (radtest, pfSense, etc.) sans devoir créer un compte à la main.
info "Création de l'utilisateur RADIUS de test (admin/admin)..."
mysql -u root -p"$DB_ROOT_PASS" "$DB_NAME" <<'SQL' 2>/tmp/test_user.log || warn "Création de l'utilisateur de test échouée (voir /tmp/test_user.log)."
DELETE FROM radcheck WHERE username = 'admin' AND attribute = 'Cleartext-Password';
INSERT INTO radcheck (username, attribute, op, value) VALUES ('admin', 'Cleartext-Password', ':=', 'admin');
SQL
warn "Utilisateur RADIUS de test créé : admin / admin — pense à le changer ou le supprimer avant la mise en production !"
...
if [ -f "3.0.zip" ]; then
    ...
    if [ -f "$SQL_MOD" ]; then
        ...
        sed -i -E "s/^([[:space:]]*login[[:space:]]*=[[:space:]]*).*/\1\"${DB_APP_USER_DQ}\"/" "$SQL_MOD"
        sed -i -E "s/^([[:space:]]*password[[:space:]]*=[[:space:]]*).*/\1\"${DB_APP_PASS_DQ}\"/" "$SQL_MOD"
        sed -i -E "s/^([[:space:]]*radius_db[[:space:]]*=[[:space:]]*).*/\1\"${DB_NAME_DQ}\"/" "$SQL_MOD"
    ...
    if [ -f "$CLIENTS_CONF" ]; then
        ...
        sed -i -E "s/^([[:space:]]*secret[[:space:]]*=[[:space:]]*).*/\1\"${RADIUS_SECRET_DQ}\"/" "$CLIENTS_CONF"
...
if [ -d "radius-dashboard" ]; then
    ...
    if [ -f "/var/www/html/radius-dashboard/config.php" ]; then
        ...
        sed -i -E "s/(define\\([[:space:]]*'DB_NAME'[[:space:]]*,[[:space:]]*)'[^']*'/\\1'${DB_NAME_SQ}'/" \
            /var/www/html/radius-dashboard/config.php || true
        sed -i -E "s/(define\\([[:space:]]*'DB_USER'[[:space:]]*,[[:space:]]*)'[^']*'/\\1'${DB_APP_USER_SQ}'/" \
            /var/www/html/radius-dashboard/config.php || true
        sed -i -E "s/(define\\([[:space:]]*'DB_PASS'[[:space:]]*,[[:space:]]*)'[^']*'/\\1'${DB_APP_PASS_SQ}'/" \
            /var/www/html/radius-dashboard/config.php || true
        ...
        sed -i -E "s/^([[:space:]]*\\\$DB_NAME[[:space:]]*=[[:space:]]*).*/\\1'${DB_NAME_SQ}';/" \
            /var/www/html/radius-dashboard/config.php || true
        sed -i -E "s/^([[:space:]]*\\\$DB_USER[[:space:]]*=[[:space:]]*).*/\\1'${DB_APP_USER_SQ}';/" \
            /var/www/html/radius-dashboard/config.php || true
        sed -i -E "s/^([[:space:]]*\\\$DB_PASS(WORD|word|Word)?[[:space:]]*=[[:space:]]*).*/\\1'${DB_APP_PASS_SQ}';/" \
            /var/www/html/radius-dashboard/config.php || true
    else
...
SSH_PORT="$(grep -E '^[[:space:]]*Port[[:space:]]+[0-9]+' /etc/ssh/sshd_config 2>/dev/null | awk '{print $2}' | tail -n1 || true)"
...
echo "   - Secret RADIUS     : $RADIUS_SECRET"
echo "                         (à copier EXACTEMENT dans pfSense -> RADIUS Shared Secret)"
echo "   - Utilisateur test  : admin / admin (RADIUS) — à changer avant la prod !"
echo "                         Test rapide : radtest admin admin localhost 0 \"$RADIUS_SECRET\""
