...
# MariaDB/Ubuntu utilise souvent unix_socket pour root.
# On configure root par ALTER USER après connexion locale.
...
if [ -f "radius.sql" ]; then
    mysql --protocol=socket -u root -p"$DB_ROOT_PASS" "$DB_NAME" < radius.sql
    info "Base de données importée avec succès."
else
    warn "Aucun fichier radius.sql trouvé dans le dépôt, étape ignorée."
fi
